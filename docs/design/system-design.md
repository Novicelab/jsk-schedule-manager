...
| 항목 | 선택 | 선택 이유 |
|------|------|-----------|
| DB | **Supabase (PostgreSQL)** | BaaS, PostgreSQL 기반 클라우드 관리형 DB, 별도 DB 서버 운영 불필요 |
| 빌드 도구 | **Gradle** | Spring Boot 기본 권장, 빌드 속도 우수 |
| 프론트엔드 | **React** | 캘린더 SPA UX, FullCalendar React 래퍼, JWT 인증 친화적 |
| 배포 | **Render** | 클라우드 앱 호스팅 플랫폼, GitHub 연동 자동 배포 |
